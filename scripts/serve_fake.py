...
def seed() -> tuple[FakeImmich, str, str, DedupConfig]:
    ...
    for i in range(3):
        ...
        fake.add_asset(p_id, f"plain-{i}", file_name=f"IMG_{2000 + i}.jpg", created_at=taken_at, size_bytes=3_000_000)
        fake.add_asset(s_id, f"plain-{i}", file_name=f"IMG_{2000 + i}.jpg", created_at=taken_at, size_bytes=3_000_000)

    # duplicate pair shared via an album owned by each user
    beach_taken_at = now - timedelta(days=30)
    keeper = fake.add_asset(p_id, "album-1", file_name="beach.jpg", created_at=beach_taken_at, size_bytes=5_500_000)
    loser = fake.add_asset(s_id, "album-1", file_name="beach.jpg", created_at=beach_taken_at, size_bytes=5_500_000)
